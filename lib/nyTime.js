...
const nyShortTimeFormatter = new Intl.DateTimeFormat("en-US", {
  timeZone: APP_TIME_ZONE,
  hour: "numeric",
  minute: "2-digit",
  hour12: true,
...
/**
 * Short clock time in Eastern, e.g. "1:53pm".
 * @param {string | Date | number} value
 * @returns {string}
 */
export function formatTimeShortNy(value) {
  if (!value) {
    return "—";
  }
  const formatted = nyShortTimeFormatter.format(new Date(value));
  return formatted.replace(/\s/g, "").toLowerCase();
}

/**
 * Zero-padded HH:MM:SS countdown from milliseconds.
 * @param {number | null | undefined} ms
 * @returns {string}
 */
export function formatHmsCountdown(ms) {
  if (ms === null || ms === undefined || ms <= 0) {
    return "00:00:00";
  }
  const totalSec = Math.floor(ms / 1000);
  const hours = Math.min(99, Math.floor(totalSec / 3600));
