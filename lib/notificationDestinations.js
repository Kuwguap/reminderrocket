...
/**
 * Rows for UI: where reminder alerts are delivered (email, SMS, WhatsApp, Telegram).
 * Phone, WhatsApp, and email may contain a comma-separated list — each recipient gets
 * its own row.
 * @param {{ email?: string | null, phone?: string | null, whatsapp?: string | null, telegram_chat_id?: number | string | null }} reminder
...
export function getNotificationDestinationRows(reminder) {
  ...
  for (const number of splitRecipients(reminder.whatsapp)) {
    rows.push({ key: `whatsapp:${number}`, label: "WhatsApp", value: number });
  }
