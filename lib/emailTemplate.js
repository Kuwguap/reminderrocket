...
export function buildReminderEmail({
  ...
  const inlineImageBlock = inlineImageCid
    ? `<div style="margin-top: 12px;">
        <img src="cid:${escapeHtml(inlineImageCid)}" alt="Reminder image" style="display: block; max-width: 100%; border-radius: 12px; border: 1px solid #fed7aa;" />
      </div>`
    : "";

  const inlineVoiceBlock = inlineVoiceCid
    ? `<div style="margin-top: 12px; padding: 12px; border: 1px solid #fed7aa; border-radius: 12px; background: #fff;">
        <p style="margin: 0 0 8px; font-size: 12px; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: 0.08em;">Voice note</p>
        <audio controls src="cid:${escapeHtml(inlineVoiceCid)}" style="width: 100%;"></audio>
      </div>`
    : "";
